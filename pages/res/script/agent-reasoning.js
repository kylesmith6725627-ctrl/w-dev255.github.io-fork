...
export function createReasoner(state) {
  ...
  function remember(input, decision) {
    state.context.turns.push({ input, intent: decision.intent, category: decision.category, topic: decision.topic || null, command: decision.command, timestamp: Date.now() });
  ...
  function fallback(input, previous) {
    ...
    if (!match || !match.score) return previous && includesAny(text, ['continua', 'approfondisci'])
      ? { command: previous.command, intent: previous.intent, category: previous.category || previous.intent, confidence: 0.5, topic: previous.topic, args: [previous.topic] }
      : { command: null, intent: 'unknown', category: 'unknown', confidence: 0, topic: text };
  ...
  async function interpret(input) {
    ...
    let decision = prediction && prediction.intent !== 'unknown'
      ? { command: prediction.intent === 'continue' ? previous?.command : prediction.intent, intent: prediction.intent, category: prediction.category || prediction.intent, confidence: prediction.confidence, topic: prediction.topic, candidates: prediction.candidates }
  ...
  function contextualize(message, decision) {
    ...
    const labels = { js: `Generazione locale (confidenza ${(decision.confidence * 100).toFixed(0)}%):`, html: `HTML locale (confidenza ${(decision.confidence * 100).toFixed(0)}%):`, weather: 'Meteo:', country: 'Informazioni paese:', wiki: 'Risultato Wikimedia:' };
