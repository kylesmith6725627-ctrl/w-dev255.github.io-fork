...
export function createReasoner(state) {
  ...
  const ready = model.then((loaded) => {
    state.localModel = loaded;
    return loaded;
  });
  ...
  function fallback(input, previous) {
    ...
    const match = rules.map(([intent, command, terms]) => ({ intent, command, score: terms.filter((term) => text.includes(term)).length }))
      .sort((a, b) => b.score - a.score)[0];
    if (!match || !match.score) return previous && includesAny(text, ['continua', 'approfondisci'])
      ? { command: previous.command, intent: previous.intent, confidence: 0.5, topic: previous.topic, args: [previous.topic] }
      : { command: null, intent: 'unknown', confidence: 0, topic: text };
    return { command: match.command, intent: match.intent, confidence: 0.55 + match.score * 0.1, topic: input, args: match.command === 'js' ? [input] : [] };
  }
  ...
  async function interpret(input) {
    ...
    let decision = prediction && prediction.intent !== 'unknown'
      ? { command: prediction.intent === 'continue' ? previous?.command : prediction.intent, intent: prediction.intent, confidence: prediction.confidence, topic: prediction.topic, features: prediction.features, args: [] }
      : fallback(input, previous);
    if (decision.intent === 'continue' && previous) {
      decision.command = previous.command;
      decision.topic = previous.topic || decision.topic;
    }
    if (decision.command === 'goto') decision.args = [decision.topic];
    if (decision.command === 'js') decision.args = [decision.topic || input];
  ...
  function contextualize(message, decision) {
    ...
    const labels = { help: 'Posso aiutarti con:', time: 'Ecco data e ora locali:', history: 'Questa è la cronologia della sessione:', js: `Generazione locale (confidenza ${(decision.confidence * 100).toFixed(0)}%):`, goto: 'Navigazione richiesta:', echo: 'Come richiesto:' };
